"""Arquitectura LaMa (Fast Fourier Convolution), solo para trazar el modelo.

NO se empaqueta: vive en tools/ porque solo hace falta una vez, para
convertir el .ckpt de pesos en un TorchScript. El programa carga el
TorchScript, que lleva el grafo dentro y no necesita nada de esto.

Procedencia: recortado de manga_translator/inpainting/inpainting_lama_mpe.py
de zyddnys/manga-image-translator, que a su vez viene del LaMa original de
advimman/lama (Apache-2.0) y de la implementacion de FFC de pkumivision.
Se le ha quitado la clase asincrona del inpainter —que arrastra la config,
las utilidades y las descargas de ese proyecto— y no se ha tocado nada mas,
para que el .ckpt cargue tal cual.

OJO con la licencia si esto llega a distribuirse: manga-image-translator es
GPL-3.0 aunque el codigo de la arquitectura sea de origen Apache-2.0. Para
uso propio da igual; para publicar, conviene rehacer este archivo desde el
repo de advimman/lama y quitarse la duda.
"""
import os
import cv2
import numpy as np
